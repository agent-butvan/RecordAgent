...
- 日历周期待办便签使用 `components/calendar/StickyTodoNote` 统一实现纸张、胶带、卷角、勾选进度和动画；仅在存在真实每周或每月周期待办时显示对应便签，勾选复用待办完成接口。周、月便签默认折叠为日历右下角的进度标签，一次只展开一张；展开后可在日历工作区内拖拽，收起时回到右下角，并保留抬起、速度倾斜及松手回弹反馈。
