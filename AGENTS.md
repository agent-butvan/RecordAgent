...
- 日历周期待办便签使用 `components/calendar/StickyTodoNote` 统一实现纸张、胶带、卷角、勾选进度和动画；仅在存在真实每周或每月周期待办时显示对应便签，勾选复用待办完成接口。便签置于整个日历工作区顶层，拖拽仅从便签空白区域开始，边界不得越过左侧菜单或日历工作区，并保留抬起、速度倾斜及松手回弹反馈。
