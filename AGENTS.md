...
- 日历活力是仅用于展示的固定规则：学习每 30 分钟 10 分（上限 40），已完成待办每项 10 分（上限 30），当天资料每篇 10 分（上限 30）；收支及自动收益不计分。0 分与四档正分使用低饱和灰绿底色，未来日期不染色，数据不可用与无活动必须区分。月历统一维护一个延迟悬浮框，点击日期更新右侧详情；浮层分类折叠，右侧完整展开，共用详情组件。
