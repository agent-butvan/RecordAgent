...
- **后端 REST API 日志打印契约**：所有后端 Controller REST API 接口的调用必须经过 `ApiLogAspect` AOP 切面，进行规范化的请求参数、客户端 IP、处理状态及毫秒级耗时（Cost）日志输出。
...
- **接口日志输出**：后端每个 API 接口的调用必须包含规范化的日志输出打印（形如 `[API-LOG] START/END/ERROR`）。
